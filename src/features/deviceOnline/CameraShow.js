...
import { countOnlineCameraGroupByType } from '../../common/request'
...
const CameraShow = (props) => {
  ...
  useEffect(() => {
    countOnlineCameraGroupByType().then((data) => {
      if (data) {
        const { result } = data
            ...
            cardList.map((item, i) => (
              ...
              >
                <Card style={style} bordered={true} >
                  <div style={{ height: '35px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.item}</div>
                  <div>总数：{item.totalCount}</div>
                  <div><span className="online-text">在线：{item.onlineCount}</span>/<span className="outline-text">离线：{item.offlineCount}</span></div>
                </Card>
              </Col>
            ))
          }
